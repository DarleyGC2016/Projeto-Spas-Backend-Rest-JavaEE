...
public class EnderecoResource {
    ...
    @POST
    @Path("buscar")
    public Response obterCep(String cep) {
        return enderecoServico.buscarCep(cep).map(postal -> Response.ok(postal).build()).orElseGet(() -> Response.status(Response.Status.NOT_FOUND).build());
    }
}
